...
【AdvancedDroneSpiker】
概要:
  単体動作する旧スパイカー。MatchManager / VolleyballManager 非依存で、
  ボールが上昇中に弾道を計算し、待機 → 移動 → 打撃 → 帰還 の状態遷移でスパイクする。
  ネット回避チェックを持つが、spikeFlightTime への依存問題が残っている（コメントに記録あり）。

他スクリプトとの関係:
  ・BallToss2  ← ボール速度変換に依存していた旧構造（コメントに過去ログあり）
  ・DestoryOnCollision  ← コートへの着弾でボールが消えることを前提

【注意 ─ 削除候補】
  SpikerAllyEnemyV2 が MatchManager 連携・ボール回避付きの上位互換版のため、
  このスクリプトは開発中の実験的なものとして保持されている状態。
  DroneSpikeMultiple（複数ボール対応旧版）と設計が類似しており、
  どちらも現行システムには組み込まれていない。
*/
/*
トスされた球の弾道計算
